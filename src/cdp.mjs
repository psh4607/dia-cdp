#!/usr/bin/env node
// dia-cdp - lightweight Dia DevTools Protocol CLI
...
function sockPath(targetId) {
  return IS_WINDOWS
    ? `\\\\.\\pipe\\dia-cdp-${targetId}`
    : resolve(RUNTIME_DIR, `dia-cdp-${targetId}.sock`);
}
  ...
  async connect(wsUrl) {
    return new Promise((res, rej) => {
      ...
      const timer = setTimeout(() => {
        settle(rej, new Error(`Timeout connecting to CDP WebSocket after ${CONNECT_TIMEOUT}ms`));
        try { this.#ws?.close(); } catch {}
      }, CONNECT_TIMEOUT);

      this.#ws = new WebSocket(wsUrl);
      this.#ws.onopen = () => settle(res);
      this.#ws.onerror = (e) => settle(rej, new Error('WebSocket error: ' + (e.message || e.type)));
